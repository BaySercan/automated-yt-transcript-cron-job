...
export class YahooService {
  ...
  async getHistoryRange(
    ...
    try {
      ...
      logger.info(
        `YahooService: Fetching batch history for ${symbol} from ${
      ...
      logger.info(
        `YahooService: Found ${priceMap.size} prices for ${symbol} in range`
      );
      return priceMap;
    } catch (error: any) {
  ...
  async getHistory(symbol: string, date: Date): Promise<number | null> {
    ...
    try {
      ...
      logger.info(
        `YahooService: Fetching history for ${symbol} around ${
      ...
      for (let i = 0; i < timestamps.length; i++) {
        ...
        if (candleDateStr === targetDateStr) {
          ...
          if (closeVal !== null && closeVal !== undefined) {
            const finalPrice = adjCloseVal || closeVal;
            logger.info(
              `YahooService: Found match for ${symbol} on ${targetDateStr}: ${finalPrice}`
            );
            return finalPrice;
          }
        }
      ...
      logger.warn(
        `YahooService: No matching candle found for ${symbol} on ${targetDateStr} in fetched window`
      );
      return null;
    } catch (error: any) {
  ...
  async search(
    query: string
  ): Promise<{ symbol: string; exchange?: string; quoteType?: string } | null> {
    try {
      ...
      if (quotes && quotes.length > 0) {
        ...
        const best = quotes[0];
        logger.info(
          `YahooService: Resolved "${query}" to ${best.symbol} (${
            best.shortname || best.longname
          }) - Exch: ${best.exchange}, Type: ${best.quoteType}`
        );
        return {
          symbol: best.symbol,
