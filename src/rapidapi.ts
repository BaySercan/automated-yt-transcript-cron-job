...
export class RapidAPIService {
  ...
  async getVideoInfo(videoId: string): Promise<VideoInfo | null> {
    ...
    try {
      logger.info(
        `Fetching video info from RapidAPI /info endpoint for video ${videoId}`
      );
      ...
      return await this.circuitBreaker.execute(async () => {
        ...
        if (response.status === 429) {
          ...
          logger.warn(
            `Rate limit hit for video info request: ${retryAfter}s until reset`
          );
        ...
        logger.info(`📋 Received video info for ${videoId}:`, {
          ...
          availableFields: Object.keys(data).filter(
      ...
      throw new TranscriptError(
        `RapidAPI /info failed: ${(error as Error).message}`
      );
    }
  }
  ...
  async getVideoTranscript(videoId: string): Promise<string> {
    ...
    try {
      ...
      return await this.circuitBreaker.execute(async () => {
        ...
        logger.info(
          `RapidAPI process started with ID: ${initialResponse.process_id}`
        );
        ...
        if (
          ...
          logger.info(
            `✅ SUCCESS! Transcript ready for video ${videoId} (${result.transcript.length} characters)`
          );
        ...
        if (
          ...
          (result.isProcessed === true && !result.transcript) ||
          result.status === "failed"
        ) {
          const error = `RapidAPI processing failed: ${
  ...
  private async requestTranscript(
    videoId: string
  ): Promise<RapidAPITranscriptResponse> {
    const startTime = Date.now();
    ...
    try {
      ...
      if (response.status === 429) {
        ...
        logger.warn(
          `Rate limit hit for transcript request: ${retryAfter}s until reset`
        );

        RateLimitMonitor.recordRequest(
          "rapidapi-transcript-request",
          false,
          responseTime,
          true
        );
      ...
      if (!processId) {
        throw new TranscriptError(
          "Invalid response from RapidAPI: missing process_id/processingId"
        );
      }
      ...
      logger.error(`Transcript request failed for ${videoId}`, {
        error,
        responseTime,
        isRateLimitError,
      });
  ...
  private async pollForResult(processId: string): Promise<RapidAPIResult> {
    ...
    // Optimized for faster response when transcripts are ready quickly
    const getPollDelay = (attempt: number): number => {
      if (attempt < 3) return 10000; // First 3 attempts: 10 seconds (catch fast completions)
      if (attempt < 6) return 15000; // Next 3 attempts: 15 seconds
      if (attempt < 10) return 25000; // Next 4 attempts: 25 seconds
      if (attempt < 20) return 45000; // Next 10 attempts: 45 seconds
      return 60000; // After that: 60 seconds
    };
    ...
    while (attempts < maxAttempts) {
      ...
      if (Date.now() - startTime > maxPollTime) {
        throw new TranscriptError(
          `RapidAPI processing timeout after ${
            maxPollTime / 1000 / 60
          } minutes for process ${processId}`
        );
      }
      ...
      try {
        ...
        logger.info(
          `🔍 RAPIDAPI POLLING ATTEMPT ${
            attempts + 1
          }/${maxAttempts} for ${processId}:`,
          {
            success: result.success,
            isProcessed: result.isProcessed,
            hasTranscript: !!result.transcript,
        ...
        if (
          ...
          const totalTime = Date.now() - startTime;
          logger.info(
            `🎉 SUCCESS! RapidAPI processing completed for ${processId} after ${
              attempts + 1
            } attempts (${totalTime / 1000}s)`
          );
          logger.info(
            `📝 Transcript found: ${result.transcript.length} characters`
          );
          logger.info(
            `📄 Transcript preview: "${result.transcript.substring(0, 200)}..."`
          );
        ...
        if (
          ...
          logger.error(
            `❌ FAILED! RapidAPI processing failed for ${processId}`,
            {
              success: result.success,
              isProcessed: result.isProcessed,
              hasTranscript: !!result.transcript,
        ...
        const progress =
          (result as any).progress || (result as any).percentage || "unknown";
        const pollDelay = getPollDelay(attempts);

        logger.info(
          `⏳ STILL PROCESSING: ${processId} - Attempt ${
            attempts + 1
          }/${maxAttempts}, Progress: ${progress}%, Next poll in ${
            pollDelay / 1000
          }s`
        );
        ...
        logger.warn(
          `Error polling RapidAPI result for ${processId} (attempt ${
            attempts + 1
          }/${maxAttempts})`,
          {
            error,
    ...
    throw new TranscriptError(
      `RapidAPI processing timeout after ${maxAttempts} attempts (${Math.round(
        (Date.now() - startTime) / 1000
      )}s) for process ${processId}`
    );
  }
  ...
  private async getResult(processId: string): Promise<RapidAPIResult> {
    ...
    try {
      ...
      if (response.status === 429) {
        ...
        logger.warn(
          `Rate limit hit for result request: ${retryAfter}s until reset`
        );
      ...
      if (data.status === "failed") {
        logger.warn(
          `🚨 DETECTED FAILED STATUS in response for process ${processId}:`,
          {
            status: data.status,
      ...
      if (
        ...
        throw new TranscriptError(
          "Invalid response from RapidAPI: missing required fields"
        );
      }
      ...
      logger.error(`Result request failed for process ${processId}`, {
        error,
        responseTime,
        isRateLimitError,
      });
  ...
  getRateLimitStats(): any {
    return {
      ...
      endpointStats: {
        ...
        transcriptRequest: RateLimitMonitor.getStats(
          "rapidapi-transcript-request"
        ),
